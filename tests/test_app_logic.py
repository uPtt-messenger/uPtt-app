...
@pytest.fixture
def app_instance():
    mock_ptt_service = MagicMock(spec=PyPtt.Service)

    # Patch Buffer class so UPttApp creates mock buffers
    with patch('prompt_toolkit.buffer.Buffer', autospec=True) as MockBufferClass:
        ...
        app = UPttApp(mock_ptt_service) # Instantiate app after setting side_effect

        # Mock the app.app (prompt_toolkit Application) as well, as it's accessed during state changes
        app.app = MagicMock()
        app.app.layout = MagicMock()
        app.app.layout.focus = MagicMock()
        app.app.invalidate = MagicMock()
        app.app.exit = MagicMock() # Mock exit method

        # The app instance's buffers are already the mock objects due to MockBufferClass.side_effect
        # We just need to ensure their .text and .reset methods are properly configured.
        # MagicMock handles .text attribute setting/getting by default
...
def test_login_success(app_instance):
    ...
    app_instance.ptt_service.call.return_value = None # Simulate successful call

    app_instance.login()

    app_instance.ptt_service.call.assert_called_once_with(
        'login', {'ptt_id': "test_id", 'ptt_pw': "test_pw", 'kick_other_session': True}
    )
    assert app_instance.state == 'SELECT_TARGET'
    assert app_instance.error_message == ''
...
def test_login_wrong_id_or_password(app_instance):
    ...
    app_instance.ptt_service.call.side_effect = MockWrongIDorPassword() # Use custom mock exception

    app_instance.login()

    assert app_instance.state == 'LOGIN'
    assert app_instance.error_message == '帳號密碼錯誤'
...
def test_login_only_secure_connection(app_instance):
    ...
    app_instance.ptt_service.call.side_effect = MockOnlySecureConnection() # Use custom mock exception

    app_instance.login()

    assert app_instance.state == 'LOGIN'
    assert app_instance.error_message == '只能使用安全連線'
    app_instance.app.invalidate.assert_called_once()

def test_login_reset_contact_email(app_instance):
    app_instance.id_buffer.text = "id"
    app_instance.pw_buffer.text = "pw"
    app_instance.ptt_service.call.side_effect = MockResetYourContactEmail() # Use custom mock exception

    app_instance.login()

    assert app_instance.state == 'LOGIN'
    assert app_instance.error_message == '請先至信箱設定連絡信箱'
    app_instance.app.invalidate.assert_called_once()
...
def test_login_generic_error(app_instance):
    ...
    app_instance.ptt_service.call.side_effect = MockLoginError(generic_error_msg) # Use custom mock exception

    app_instance.login()

    assert app_instance.state == 'LOGIN'
    assert app_instance.error_message == f'登入失敗: {generic_error_msg}'
    app_instance.app.invalidate.assert_called_once()
...
def test_select_target_no_such_user(app_instance):
    ...
    assert app_instance.state == 'LOGIN' # Corrected assertion: state remains LOGIN
    assert app_instance.error_message == '查無此人'
    app_instance.app.invalidate.assert_called_once()
...
def test_send_message_success(app_instance):
    ...
    with patch('src.uPttTerm.app.utils.msg_to_mail') as mock_msg_to_mail:
        ...
        app_instance.input_buffer.reset.assert_called_once()
        mock_msg_to_mail.assert_called_once_with(app_name, ptt_id, message_text)
        app_instance.ptt_service.call.assert_called_once_with(
            'mail',
            {'ptt_id': target, 'title': contant.PTT_MSG_TITLE, 'content': "formatted_mail_content", 'backup': False}
        )
        assert (contant.USER_MSG, message_text) in app_instance.messages
        app_instance.app.invalidate.assert_called_once()
