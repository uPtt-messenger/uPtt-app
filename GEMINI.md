...
### **B. 側邊欄與視窗配置**
*   **側邊欄寬度：** 應維持精簡且不佔空間。最小寬度設定為 `160px`，最大寬度 `250px`。
*   **預設比例：** 主聊天視窗啟動後的 `QSplitter` 初始比例建議維持在 `1:3.5` 左右（例如側邊欄 180px），確保視覺重心保留在聊天區域。
...
*   **行間距：** ID 與暱稱之間的行距應保持緊湊（建議 `setSpacing(2)`），使其呈現為一個整體的訊息群組。
