...
from PySide6.QtCore import Qt, QMetaObject, QSize, QThread, QTimer, Signal, Slot
from PySide6.QtGui import QAction, QFontMetrics, QIcon, QKeySequence, QShortcut
from PySide6.QtWidgets import (
    QApplication, QHBoxLayout, QLabel, QLineEdit, QListWidgetItem,
...
from uPtt.ui._render import render_themed_icon
...
from uPtt.ui.widgets import (
    ChatBubble, ContactItem, ContactListWidget, EmptyChatPlaceholder,
    EmptySidebarPlaceholder, MailCard, MessageInput, WaterballBubble,
)
from uPtt.utils import VersionCheckWorker, decode_reply, encode_reply
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, ptt_service: UPttService, ptt_query_service: UPttService, db):
        ...
        # 設定視窗圖示（依主題著色；apply_theme() 切換主題時會重新產生）
        self.setWindowIcon(render_themed_icon("logo_icon.svg"))
        ...
        # 啟動時從 db 讀取主題並套用（LoginWindow 是 MainWindow 的子元件，
        # setStyleSheet 會一併往下套用）
        theme_name = self.db.get_config(config.SETTING_THEME, styles.DEFAULT_THEME)
    ...
    def init_ui(self):
        ...
        # 狀態指示點（顏色由 _refresh_connection_status() 依連線狀態決定，此處只是初始預設值）
        self._status_dot = QLabel("●")
        self._status_dot.hide()

        self.user_id_label = QLabel("uPtt")
        user_layout.addWidget(self._status_dot)
        ...
        # 清單為空時顯示的置中提示；與 contact_list 共用同一層版面位置，靠
        # _update_sidebar_empty_state() 依項目數互斥顯示/隱藏（隱藏的 widget 不佔版面）。
        self.sidebar_empty_state = EmptySidebarPlaceholder()

        # QListWidget 的 addItem/takeItem/clear 都會透過內部 model 發出這些訊號，
        # 藉此統一在每個新增/刪除聯絡人的路徑後自動同步空狀態，不必逐一手動呼叫。
        self.contact_list.model().rowsInserted.connect(self._update_sidebar_empty_state)
        self.contact_list.model().rowsRemoved.connect(self._update_sidebar_empty_state)
        self.contact_list.model().modelReset.connect(self._update_sidebar_empty_state)

        sidebar_vbox.addLayout(sidebar_header)
        sidebar_vbox.addWidget(self.contact_list, 1)
        sidebar_vbox.addWidget(self.sidebar_empty_state, 1)
        ...
        self.cancel_reply_btn.setFixedSize(20, 20)
        self.cancel_reply_btn.setCursor(Qt.PointingHandCursor)
        self.cancel_reply_btn.clicked.connect(self.cancel_reply)

        reply_bar_layout.addWidget(self.reply_bar_label, 1)
        reply_bar_layout.addWidget(self.cancel_reply_btn)
        ...
        # 聊天標題列在線狀態文字（顏色由 _update_chat_header()/on_online_status_updated() 依
        # 在線狀態決定，此處只是初始隱藏前的預設值）
    ...
    def init_tray(self):
        ...
        # 系統匣圖示刻意不跟隨 App 主題：托盤／選單列背景由作業系統決定，不一定與使用者選的
        # App 內部主題一致（例：App 選了淺色 bone 主題，但 macOS 選單列仍是深色），圖示顏色
        # 跟著 App 主題走很容易在系統列裡「消失」。因此固定套用 graphite 的深色調色盤，維持
        # 一致的高對比外觀，不受 apply_theme() 影響。
        tray_icon = render_themed_icon("logo_icon.svg", theme_tokens=styles.THEMES['graphite'])
    ...
    def on_login_result(self, success, message, kind=""):
        if success:
            ...
            self.login_screen.show_error(message, kind)
    ...
    def _refresh_connection_status(self):
        ...
        if not self._main_connected:
            self._status_dot.setStyleSheet(f"color: {t['warn']}; font-size: 9px; background: transparent;")
            self._status_dot.setToolTip("連線中斷，正在重新連線...")
        elif self._query_degraded:
            self._status_dot.setStyleSheet(f"color: {t['online']}; font-size: 9px; background: transparent;")
    ...
    def on_online_status_updated(self, ptt_id: str, is_online: bool):
        ...
        if self.current_chat_id == ptt_id_lower:
            t = styles.theme()
            online_color = t['online'] if is_online else t['faint']
            self.chat_header_online.setText("● 在線上" if is_online else "● 離線")
            self.chat_header_online.setStyleSheet(
                f"font-size: 10px; color: {online_color}; background: transparent;"
            )
            self.chat_header_online.show()
            self.chat_header_online_dot.setStyleSheet(
                f"background-color: {online_color}; border-radius: 5px; border: 2px solid {t['bg']};"
            )
    ...
    def _update_sidebar_empty_state(self, *_args):
        """依聯絡人清單是否為空，互斥顯示 contact_list 或 sidebar_empty_state。
        接掛在 contact_list 內部 model 的 rowsInserted/rowsRemoved/modelReset 訊號上
        （見 init_ui），涵蓋所有新增/刪除聯絡人的路徑，不需在每個呼叫點各自處理。"""
        is_empty = self.contact_list.count() == 0
        self.sidebar_empty_state.setVisible(is_empty)
        self.contact_list.setVisible(not is_empty)
    ...
    def refresh_chat_display(self):
        ...
        # 清除現有訊息。deleteLater() 是非同步的：若在同一個事件迴圈迭代內
        # 連續呼叫兩次 refresh_chat_display()（例如切換主題時 apply_theme()
        # 觸發），舊 widget 在真正被刪除前仍是 messages_widget 的可見子物件、
        # 停留在原本的幾何位置，會被新建的 widget 疊在上面，畫面出現殘影。
        # setParent(None) 會立即（同步）把 widget 從父層的子物件樹與畫面上
        # 移除，因此不會有殘影；deleteLater() 留著讓 event loop 之後再真正
        # 釋放記憶體即可，這裡的時間差已經不影響畫面。
        ...
        if not self.current_chat_id:
            self.chat_header.hide()
            # 未選任何對話：中央顯示置中空狀態，取代原本的空白畫面。EmptyChatPlaceholder
            # 在建構時就讀取當下 theme()，故每次呼叫都重建即可自動跟上主題切換。
            self.messages_layout.addStretch(1)
            self.messages_layout.addWidget(EmptyChatPlaceholder())
            self.messages_layout.addStretch(1)
            return
    ...
    def _highlight_widget(self, widget):
        ...
        widget.setStyleSheet(f"background-color: {styles.theme()['selection']}; border-radius: 6px;")
    ...
    def _apply_settings(self):
        """將最新的輪詢間隔設定套用到執行中的 worker 計時器（跨執行緒安全），並套用主題變更。"""
        ...
        theme_name = self.db.get_config(config.SETTING_THEME, styles.DEFAULT_THEME)
        if theme_name != styles.theme().get('id'):
            self.apply_theme(theme_name)

    def apply_theme(self, name: str):
        """切換主題：套用新 QSS，並重建含 inline 樣式、不受父層 stylesheet 影響的元件
        （登入畫面／掃描畫面／聊天版面主體／聯絡人卡片 / 對話氣泡 / 視窗與 logo 圖示）。"""
        styles.set_theme(name)
        self.setStyleSheet(styles.build_style())
        self.login_screen.refresh_theme()
        self.scan_setup_screen.refresh_theme()
        self._refresh_static_theme_colors()
        self._refresh_connection_status()
        self._refresh_contact_list_theme()
        self.refresh_chat_display()
        self.setWindowIcon(render_themed_icon("logo_icon.svg"))

    def _refresh_static_theme_colors(self):
        """重新套用聊天版面主體中「用 setStyleSheet 直接設色、不受父層 QSS 影響」的
        inline 樣式（側邊欄 splitter/使用者列/搜尋輸入框/回覆列/聊天標題列）。
        在線狀態相關的顏色（連線狀態點、聊天標題列在線指示）分別由
        _refresh_connection_status() 與 _update_chat_header()/on_online_status_updated()
        依當下狀態決定，不在這裡處理。"""
        t = styles.theme()
        self.splitter.setStyleSheet(f"QSplitter::handle {{ background-color: {t['border']}; }}")
        self.user_profile.setStyleSheet(f"""
            background-color: {t['panel']};
            border-bottom: 1px solid {t['divider']};
        """)
        self.user_id_label.setStyleSheet(
            f"font-weight: bold; font-size: 13px; color: {t['ink']}; background: transparent;"
        )
        ...
        """)
        self.reply_bar_label.setStyleSheet(f"color: {t['muted']}; font-size: 12px;")
        self.cancel_reply_btn.setStyleSheet(f"""
            QPushButton {{ color: {t['muted']}; background: transparent; border: none; font-size: 14px; }}
            QPushButton:hover {{ color: {t['ink2']}; }}
        """)
        self.chat_header_avatar.setStyleSheet(f"""
        ...
        self.chat_header_id.setStyleSheet(
            f"font-weight: bold; font-size: 14px; color: {t['ink']}; background: transparent;"
        )
        self.chat_header_nick.setStyleSheet(
            f"font-size: 11px; color: {t['muted']}; background: transparent;"
        )

    def _refresh_contact_list_theme(self):
        """主題切換後重建所有聯絡人卡片（ContactItem 用的是建構時固定的 inline 樣式，
        父層 stylesheet 蓋不到，需整個 widget 換新）。"""
        self.sidebar_empty_state.refresh_theme()
        for i in range(self.contact_list.count()):
            item = self.contact_list.item(i)
            widget = self.contact_list.itemWidget(item)
            if widget is None:
            ...
            new_item = self._rebuild_contact_item(data, i, data['is_pinned'])
            if was_selected:
